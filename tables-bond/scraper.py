...
data = (
    bond_data.assign(
        ...
        year=lambda data:pd.to_numeric(
            data["year"],
            errors="coerce"
        ),
    )
...
data.columns = [col if isinstance(col, str) else new_column_names.get(col, f"{col[0]}_{col[1]}") for col in data.columns]
